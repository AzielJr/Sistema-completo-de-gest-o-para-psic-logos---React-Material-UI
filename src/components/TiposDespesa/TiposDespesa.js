...
const TiposDespesa = () => {
  ...
  const handleOpen = (tipo = null) => {
    if (tipo) {
      ...
      setFormData({ ds_tipo: tipo.ds_tipo });
    } else {
      setEditingId(null);
      setFormData({ ds_tipo: '' });
    }
  ...
  const handleSubmit = (e) => {
    ...
    if (!formData.ds_tipo.trim()) {
      setError('Por favor, preencha o tipo de despesa');
      return;
    }
    ...
    if (editingId) {
      ...
      setTipos(tipos.map(tipo => 
        tipo.id === editingId 
          ? { ...tipo, ds_tipo: formData.ds_tipo.trim() }
          : tipo
      ));
    } else {
      // Adicionar
  ...
  const handleDelete = (id) => {
    if (window.confirm('Tem certeza que deseja excluir este tipo de despesa?')) {
            ...
            <Table>
              <TableHead>
                <TableRow sx={{ backgroundColor: theme.palette.grey[50] }}>
                  ...
                  <TableCell sx={{ fontWeight: 600, fontSize: '0.95rem' }}>Tipo de Despesa</TableCell>
        ...
        <form onSubmit={handleSubmit}>
          <DialogContent>
            ...
            <TextField
              ...
              margin="normal"
              variant="outlined"
              placeholder="Ex: Material, Aluguel, Equipamentos..."
              sx={{
